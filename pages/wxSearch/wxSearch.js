...
function wxSearchKeyTap(e) {
  console.log("wxSearchKey Tap: ",e)
  search(null, e.target.dataset.key.Id);
...
function wxSearchConfirm(e) {
  ...
  if (key == 'back') {
    ...
    search(__that.data.wxSearchData.value, null);
  }
}

function __goBackFunction(){
  wx.navigateBack({delta: 1})
}

function search(inputValue, id) {
  __searchFunction(inputValue, id);
  /*
  if (inputValue && inputValue.length > 0) {
    // 添加历史记录
